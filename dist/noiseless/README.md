...
3. In LM Studio, load a model capable of structured JSON output and start the server from **Developer**. In Setup's Source mode, set `providers.lmstudio.model` to the model identifier. The default endpoint is `http://127.0.0.1:1234/v1`.
...
In `Noiseless/Setup.md` Source mode:

- Set provider `enabled: true`, the exact model ID, and current USD input/output prices per million tokens.
- Set a positive `monthlyCloudBudget`. Zero pauses cloud processing.
...
Priority is the normalized weighted mean of urgency (25), L1 alignment (30), expected impact (20), ROI (20), and reputation (5). Edit these relative weights in Setup. Expand any task to see every contribution to its priority.
